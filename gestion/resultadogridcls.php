...
$resultado_grid = NULL; // Initialize page object first

class cresultado_grid extends cresultado {
	...
	// Page object name
	var $PageObjName = 'resultado_grid';

	// Grid form hidden field names
	var $FormName = 'fresultadogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["resultado"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idResultado->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_objetivo") && $objForm->HasValue("o_objetivo") && $this->objetivo->CurrentValue <> $this->objetivo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nombre") && $objForm->HasValue("o_nombre") && $this->nombre->CurrentValue <> $this->nombre->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tiempoEstimado") && $objForm->HasValue("o_tiempoEstimado") && $this->tiempoEstimado->CurrentValue <> $this->tiempoEstimado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tiempoTipo") && $objForm->HasValue("o_tiempoTipo") && $this->tiempoTipo->CurrentValue <> $this->tiempoTipo->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idResultado->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->idResultado->CurrentValue = NULL;
		$this->idResultado->OldValue = $this->idResultado->CurrentValue;
		...
		$this->tiempoEstimado->OldValue = $this->tiempoEstimado->CurrentValue;
		$this->tiempoTipo->CurrentValue = "Dia";
		$this->tiempoTipo->OldValue = $this->tiempoTipo->CurrentValue;
		...
		$this->estatus->CurrentValue = "Sin Avance";
	...
	function LoadFormValues() {
		...
		if (!$this->idResultado->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idResultado->setFormValue($objForm->GetValue("x_idResultado"));
		if (!$this->objetivo->FldIsDetailKey) {
			$this->objetivo->setFormValue($objForm->GetValue("x_objetivo"));
		}
		$this->objetivo->setOldValue($objForm->GetValue("o_objetivo"));
		...
		$this->tiempoEstimado->setOldValue($objForm->GetValue("o_tiempoEstimado"));
		if (!$this->tiempoTipo->FldIsDetailKey) {
			$this->tiempoTipo->setFormValue($objForm->GetValue("x_tiempoTipo"));
		}
		$this->tiempoTipo->setOldValue($objForm->GetValue("o_tiempoTipo"));
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idResultado->CurrentValue = $this->idResultado->FormValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idResultado->CurrentValue = strval($arKeys[0]); // idResultado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// objetivo
			$this->objetivo->SetDbValueDef($rsnew, $this->objetivo->CurrentValue, 0, $this->objetivo->ReadOnly);

			// nombre
			$this->nombre->SetDbValueDef($rsnew, $this->nombre->CurrentValue, "", $this->nombre->ReadOnly);

			// tiempoEstimado
			$this->tiempoEstimado->SetDbValueDef($rsnew, $this->tiempoEstimado->CurrentValue, 0, $this->tiempoEstimado->ReadOnly);

			// tiempoTipo
			$this->tiempoTipo->SetDbValueDef($rsnew, $this->tiempoTipo->CurrentValue, "", $this->tiempoTipo->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "objetivo") {
			...
			if ($GLOBALS["objetivo"]->EventCancelled) $this->EventCancelled = TRUE;
		}
