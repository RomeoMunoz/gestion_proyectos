...
$objetivo_view = NULL; // Initialize page object first

class cobjetivo_view extends cobjetivo {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["idObjetivo"] <> "") {
			$this->RecKey["idObjetivo"] = $_GET["idObjetivo"];
			$KeyUrl .= "&amp;idObjetivo=" . urlencode($this->RecKey["idObjetivo"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["idObjetivo"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["idObjetivo"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idObjetivo"] <> "") {
				...
				$this->RecKey["idObjetivo"] = $this->idObjetivo->QueryStringValue;
			} elseif (@$_POST["idObjetivo"] <> "") {
				$this->idObjetivo->setFormValue($_POST["idObjetivo"]);
				$this->RecKey["idObjetivo"] = $this->idObjetivo->FormValue;
			} else {
				$sReturnUrl = "objetivolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "objetivolist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "objetivolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_resultado"
		$item = &$option->Add("detail_resultado");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("resultado", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["resultado_grid"] && $GLOBALS["resultado_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'resultado')) {
		...
		if ($GLOBALS["resultado_grid"] && $GLOBALS["resultado_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'resultado')) {
		...
		if ($GLOBALS["resultado_grid"] && $GLOBALS["resultado_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'resultado')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'resultado');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (resultado)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("resultado", explode(",", $this->getCurrentDetailTable()))) {
			global $resultado;
			if (!isset($resultado)) $resultado = new cresultado;
			$rsdetail = $resultado->LoadRs($resultado->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$resultado->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("resultado", $DetailTblVar)) {
				...
				if ($GLOBALS["resultado_grid"]->DetailView) {
					$GLOBALS["resultado_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($objetivo_view)) $objetivo_view = new cobjetivo_view();

// Page init
$objetivo_view->Page_Init();

// Page main
$objetivo_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$objetivo_view->Page_Render();
?>
...
var CurrentForm = fobjetivoview = new ew_Form("fobjetivoview", "view");

// Form_CustomValidate event
fobjetivoview.Form_CustomValidate = 
...
fobjetivoview.ValidateRequired = true;
<?php } else { ?>
fobjetivoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fobjetivoview.Lists["x_formatoDuracion"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fobjetivoview.Lists["x_formatoDuracion"].Options = <?php echo json_encode($objetivo->formatoDuracion->Options()) ?>;
fobjetivoview.Lists["x_proyecto"] = {"LinkField":"x_idProyecto","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fobjetivoview.Lists["x_tipo"] = {"LinkField":"x_idObjetivosTipo","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fobjetivoview.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fobjetivoview.Lists["x_estado"].Options = <?php echo json_encode($objetivo->estado->Options()) ?>;
...
<?php } ?>
<?php $objetivo_view->ExportOptions->Render("body") ?>
<?php
	foreach ($objetivo_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($objetivo->Export == "") { ?>
...
<?php } ?>
<?php $objetivo_view->ShowPageHeader(); ?>
<?php
$objetivo_view->ShowMessage();
?>
<form name="fobjetivoview" id="fobjetivoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($objetivo_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $objetivo_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="objetivo">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($objetivo->idObjetivo->Visible) { // idObjetivo ?>
	<tr id="r_idObjetivo">
		<td><span id="elh_objetivo_idObjetivo"><?php echo $objetivo->idObjetivo->FldCaption() ?></span></td>
...
<?php echo $objetivo->idObjetivo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($objetivo->nombre->Visible) { // nombre ?>
	<tr id="r_nombre">
		<td><span id="elh_objetivo_nombre"><?php echo $objetivo->nombre->FldCaption() ?></span></td>
...
<?php echo $objetivo->nombre->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $objetivo->comentarios->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($objetivo->duracion->Visible) { // duracion ?>
	<tr id="r_duracion">
		<td><span id="elh_objetivo_duracion"><?php echo $objetivo->duracion->FldCaption() ?></span></td>
...
<?php echo $objetivo->duracion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $objetivo->formatoDuracion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($objetivo->fechaInicio->Visible) { // fechaInicio ?>
	<tr id="r_fechaInicio">
		<td><span id="elh_objetivo_fechaInicio"><?php echo $objetivo->fechaInicio->FldCaption() ?></span></td>
...
<?php echo $objetivo->fechaInicio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($objetivo->fechFin->Visible) { // fechFin ?>
	<tr id="r_fechFin">
		<td><span id="elh_objetivo_fechFin"><?php echo $objetivo->fechFin->FldCaption() ?></span></td>
...
<?php echo $objetivo->fechFin->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($objetivo->proyecto->Visible) { // proyecto ?>
	<tr id="r_proyecto">
		<td><span id="elh_objetivo_proyecto"><?php echo $objetivo->proyecto->FldCaption() ?></span></td>
...
<?php echo $objetivo->proyecto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $objetivo->tipo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($objetivo->estado->Visible) { // estado ?>
	<tr id="r_estado">
		...
		<td data-name="estado"<?php echo $objetivo->estado->CellAttributes() ?>>
<span id="el_objetivo_estado">
<span<?php echo $objetivo->estado->ViewAttributes() ?>>
<?php echo $objetivo->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("resultado", explode(",", $objetivo->getCurrentDetailTable())) && $resultado->DetailView) {
?>
