...
$objetivo_grid = NULL; // Initialize page object first

class cobjetivo_grid extends cobjetivo {
	...
	// Page object name
	var $PageObjName = 'objetivo_grid';

	// Grid form hidden field names
	var $FormName = 'fobjetivogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["objetivo"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idObjetivo->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nombre") && $objForm->HasValue("o_nombre") && $this->nombre->CurrentValue <> $this->nombre->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_comentarios") && $objForm->HasValue("o_comentarios") && $this->comentarios->CurrentValue <> $this->comentarios->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_duracion") && $objForm->HasValue("o_duracion") && $this->duracion->CurrentValue <> $this->duracion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_formatoDuracion") && $objForm->HasValue("o_formatoDuracion") && $this->formatoDuracion->CurrentValue <> $this->formatoDuracion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fechaInicio") && $objForm->HasValue("o_fechaInicio") && $this->fechaInicio->CurrentValue <> $this->fechaInicio->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fechFin") && $objForm->HasValue("o_fechFin") && $this->fechFin->CurrentValue <> $this->fechFin->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_proyecto") && $objForm->HasValue("o_proyecto") && $this->proyecto->CurrentValue <> $this->proyecto->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idObjetivo->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->idObjetivo->CurrentValue = NULL;
		$this->idObjetivo->OldValue = $this->idObjetivo->CurrentValue;
		...
		$this->formatoDuracion->CurrentValue = "Dia";
		$this->formatoDuracion->OldValue = $this->formatoDuracion->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->idObjetivo->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->comentarios->setOldValue($objForm->GetValue("o_comentarios"));
		...
		$this->formatoDuracion->setOldValue($objForm->GetValue("o_formatoDuracion"));
		...
		$this->fechFin->setOldValue($objForm->GetValue("o_fechFin"));
		if (!$this->proyecto->FldIsDetailKey) {
			$this->proyecto->setFormValue($objForm->GetValue("x_proyecto"));
		}
		$this->proyecto->setOldValue($objForm->GetValue("o_proyecto"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idObjetivo->CurrentValue = strval($arKeys[0]); // idObjetivo
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!ew_CheckInteger($this->idObjetivo->FormValue)) {
			ew_AddMessage($gsFormError, $this->idObjetivo->FldErrMsg());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// formatoDuracion
		$this->formatoDuracion->SetDbValueDef($rsnew, $this->formatoDuracion->CurrentValue, NULL, strval($this->formatoDuracion->CurrentValue) == "");
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "proyecto") {
			...
			if ($GLOBALS["proyecto"]->EventCancelled) $this->EventCancelled = TRUE;
		}
