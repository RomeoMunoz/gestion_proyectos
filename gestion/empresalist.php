...
fempresalist.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fempresalist.Lists["x_estado"].Options = <?php echo json_encode($empresa->estado->Options()) ?>;
...
<?php
	...
	if ($empresa->CurrentAction == "" && $empresa_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$empresa_list->setWarningMessage(ew_DeniedMsg());
	...
	if ($empresa_list->AuditTrailOnSearch && $empresa_list->Command == "search" && !$empresa_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $empresa_list->getSessionWhere();
		$empresa_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
...
<option value="50"<?php if ($empresa_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
...
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_idEmpresa" class="empresa_idEmpresa">
...
<span id="el<?php echo $empresa_list->RowCnt ?>_empresa_estado" class="empresa_estado">
